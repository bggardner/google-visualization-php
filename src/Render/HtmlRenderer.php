...
class HtmlRenderer
{
    protected const DETAILED_MESSAGE_A_TAG_REGEXP =
        '/([^<]*<a(( )*target="_blank")*(( )*target=\'_blank\')*(( )*href="[^"]*")*(( )*href=\'[^\']*\')*>[^<]*<\/a>)'
        . '+[^<]*/i';
    protected const BAD_JAVASCRIPT_REGEXP = '/javascript(( )*):/i';
        ...
        foreach ($dataTable->getWarnings() as $warning) {
            ...
            $messageElement->appendChild(
                $document->createTextNode(
                    ReasonType::getMessageForReasonType($warning->getReasonType()) . ". " . $warning->getMessage()
                )
            );
    ...
    protected static function sanitizeDetailedMessage($detailedMessage)
    {
        ...
        if (
            preg_match(self::DETAILED_MESSAGE_A_TAG_REGEXP, $detailedMessage)
            && !preg_match(self::BAD_JAVASCRIPT_REGEXP, $detailedMessage)
        ) {
