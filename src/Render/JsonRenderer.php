...
class JsonRenderer
{
        ...
        if ($statusType != StatusType::OK) {
            if ($statusType == StatusType::WARNING) {
                ...
                $response->errors = array(
                    self::getFaultObject($responseStatus->getReasonType(), $responseStatus->getDescription())
                );
            }
        }
    ...
    public static function renderDataTable(
        ...
        return json_encode(
            self::objectifyDataTable($dataTable, $includeValues, $includeFormatting, $renderDateAsDateConstructor),
            JSON_NUMERIC_CHECK
        );
    }

    protected static function objectifyDataTable(
        DataTable $dataTable,
        $includeValues,
        $includeFormatting,
        $renderDateAsDateConstructor
