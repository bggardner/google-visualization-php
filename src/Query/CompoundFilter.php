...
{
    protected const LOGICAL_OPERATOR_AND = 'and';
    protected const LOGICAL_OPERATOR_OR = 'or';
    ...
    public function isMatch(DataTable $table, TableRow $row)
    {
        ...
        foreach ($this->subFilters as $subFilter) {
            ...
            if (
                (($this->operator == self::LOGICAL_OPERATOR_AND) && !$result)
                || (($this->operator == self::LOGICAL_OPERATOR_OR) && $result)
            ) {
                return $result;
            }
        }
