...
final class QueryEngine
{
    ...
    protected static function performSelection(
        DataTable $table,
        Query $query,
        ColumnIndices $columnIndices,
        TreeMap $columnLookups
    ) {
        ...
        foreach ($table->getRows() as $sourceRow) {
            ...
            foreach ($selectedColumns as $col) {
                ...
                foreach ($pivotValuesSet as $values) {
                    if (
                        $columnLookups->get($values)->containsColumn($col)
                        && ((count($col->getAllAggregationColumns()) != 0) || !$wasFound)
                    ) {
                        $wasFound = true;
    ...
    protected static function performGroupingAndPivoting(
        DataTable $table,
        Query $query,
        ColumnIndices $columnIndices,
        TreeMap $columnLookups
        ...
        if (!is_null($group)) {
            $groupAndPivotScalarFunctionColumns = array_merge(
                $groupAndPivotScalarFunctionColumns,
                $group->getScalarFunctionColumns()
            );
        }
        if (!is_null($pivot)) {
            $groupAndPivotScalarFunctionColumns = array_merge(
                $groupAndPivotScalarFunctionColumns,
                $pivot->getScalarFunctionColumns()
            );
        }
        ...
        foreach ($columnAggregations as $columnAggregation) {
            foreach ($paths as $path) {
                ...
                $metaTable->put(
                    ...
                    $columnTitle,
                    new TableCell(
                        $aggregator->getAggregationValue(
                            $path,
        ...
        foreach ($rowTitles as $rowTitle) {
            ...
            foreach ($columnTitles as $colTitle) {
                ...
                $curRow->addCell(
                    !is_null($cell) ?
                        $cell
                        : new TableCell(
                            Value::getNullValueFromValueType($colDescs[$i + count($rowTitle->values)]) . getType()
                        )
                );
                $i++;
            }
            foreach ($scalarFunctionColumnTitles as $columnTitle) {
                $curRow->addCell(
                    new TableCell(
                        $columnTitle->scalarFunctionColumn->getValue(
                            $columnLookups->get($columnTitle->getValues()),
                            $curRow
                        )
                    )
                );
