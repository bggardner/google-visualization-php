...
class Query
{
    ...
    protected function checkForDuplicates($selectionColumns, $clauseName)
    {
        for ($i = 0; $i < count($selectionColumns); $i++) {
            ...
            for ($j = $i + 1; $j < count($selectionColumns); $j++) {
                if (
                    ($isCol = ($col instanceof AbstractColumn && $col->equals($selectionColumns[$j])))
                    || $col == $selectionColumns[$j]
                ) {
                    $args = array($isCol ? $col->toString() : $col, $clauseName);
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::COLUMN_ONLY_ONCE,
                        $this->localeForUserMessages,
                        $args
                    );
    ...
    public function validate()
    {
        ...
        $selectedScalarFunctionColumns = $this->hasSelection() ?
            $this->selection->getScalarFunctionColumns() :
            array();
        ...
        if ($this->hasGroup()) {
            foreach ($this->group->getColumns() as $column) {
                if (count($column->getAllAggregationColumns())) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::CANNOT_BE_IN_GROUP_BY,
                        $this->localeForUserMessages,
        ...
        if ($this->hasPivot()) {
            foreach ($this->pivot->getColumns() as $column) {
                if (count($column->getAllAggregationColumns())) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::CANNOT_BE_IN_PIVOT,
                        $this->localeForUserMessages,
                        $column->toQueryString()
                    );
                    //$log->error($messageToLogAndUser);
        ...
        if ($this->hasFilter()) {
            ...
            if (count($filterAggregations)) {
                $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                    MessagesEnum::CANNOT_BE_IN_WHERE,
                    $this->localeForUserMessages,
                    $filterAggregations[0]->toQueryString()
                );
                //$log->error($messageToLogAndUser);
                throw new InvalidQueryException($messageToLogAndUser);
            }
        ...
        foreach ($selectionSimple as $column1) {
            ...
            foreach ($selectionAggregated as $column2) {
                if ($id == $column2->getAggregatedColumn()->getId()) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::SELECT_WITH_AND_WITHOUT_AGG,
                        $this->localeForUserMessages,
                        $id
                    );
                    //$log->error($messageToLogAndUser);
        ...
        // (either grouped-by or a scalar function with valid column arguments)
        ...
        if ($this->hasSelection() && $this->hasGroup()) {
            foreach ($selectionAggregated as $column) {
                ...
                if (in_array($id, $groupColumnIds)) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::COL_AGG_NOT_IN_SELECT,
                        $this->localeForUserMessages,
                        $id
                    );
                    //$log->error($messageToLogAndUser);
        ...
        if ($this->hasGroup() && count($selectionAggregated) == 0) {
            $messageToLogAndUser = MessagesEnum::getMessage(
                MessagesEnum::CANNOT_GROUP_WITHOUT_AGG,
                $this->localForUserMessages
            );
            //$log->error($messageToLogAndUser);
            throw new InvalidQueryException($messageToLogAndUser);
        }
        if ($this->hasPivot() && count($selectionAggregated) == 0) {
            $messageToLogAndUser = MessagesEnum::getMessage(
                MessagesEnum::CANNOT_PIVOT_WITHOUT_AGG,
                $this->localForUserMessages
            );
            //$log->error($messageToLogAndUser);
            throw new InvalidQueryException($messageToLogAndUser);
        ...
        if ($this->hasSort() && count($selectionAggregated) > 0) {
            foreach ($this->sort->getColumns() as $column) {
                $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                    MessagesEnum::COL_IN_ORDER_MUST_BE_IN_SELECT,
                    $this->localeForUserMessages,
                    $column->toQueryString()
                );
        ...
        if ($this->hasPivot()) {
            foreach ($selectionAggregated as $column) {
                ...
                if (in_array($id, $pivotColumnIds)) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::AGG_IN_SELECT_NO_PIVOT,
                        $this->localeForUserMessages,
                        $id
                    );
                    //$log->error($messageToLogAndUser);
        ...
        if ($this->hasGroup() && $this->hasPivot()) {
            foreach ($groupColumnIds as $id) {
                if (in_array($id, $pivotColumnIds)) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::NO_COL_IN_GROUP_AND_PIVOT,
                        $this->localeForUserMessages,
                        $id
                    );
                    //$log->error($messageToLogAndUser);
        ...
        if ($this->hasPivot() && count($sortAggregated) > 0) {
            ...
            $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                MessagesEnum::NO_AGG_IN_ORDER_WHEN_PIVOT,
                $this->localeForUserMessages,
                $column->getAggregatedColumn()->getId()
            );
            //$log->error($messageToLogAndUser);
            throw new InvalidQueryException($messageToLogAndUser);
        }
        ...
        foreach ($sortAggregated as $column) {
            $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                MessagesEnum::AGG_IN_ORDER_NOT_IN_SELECT,
                $this->localeForUserMessages,
                $column->toQueryString()
            );
        ...
        if ($this->hasSelection()) {
            foreach ($labelColumns as $col) {
                if (!in_array($col, $selectionColumns)) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::LABEL_COL_NOT_IN_SELECT,
                        $this->localeForUserMessages,
                        $col->toQueryString()
                    );
                    //$log->error($messageToLogAndUser);
            ...
            foreach ($formatColumns as $col) {
                if (!in_array($col, $selectionColumns)) {
                    $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                        MessagesEnum::FORMAT_COL_NOT_IN_SELECT,
                        $this->localeForUserMessages,
                        $col->toQueryString()
                    );
                    //$log->error($messageToLogAndUser);
    ...
    public function getAllScalarFunctionsColumns()
    {
        ...
        if ($this->hasSelection()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->selection->getScalarFunctionColumns()
            );
        }
        if ($this->hasFilter()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->filter->getScalarFunctionColumns()
            );
        }
        if ($this->hasGroup()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->group->getScalarFunctionColumns()
            );
        }
        if ($this->hasPivot()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->pivot->getScalarFunctionColumns()
            );
        }
        if ($this->hasSort()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->sort->getScalarFunctionColumns()
            );
        }
        if ($this->hasLabels()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->labels->getScalarFunctionColumns()
            );
        }
        if ($this->hasUserFormatOptions()) {
            $mentionedScalarFunctionsColumns = array_merge(
                $mentionedScalarFunctionsColumns,
                $this->userFormatOptions->getScalarFunctionColumns()
            );
        }
    ...
    protected function checkColumnInList($columns, $column, $messageToLogAndUser)
    {
        if (in_array($column, $columns)) {
            return;
        } elseif ($column instanceof ScalarFunctionColumn) {
    ...
    protected function checkSelectedColumnWithGrouping($groupColumns, AbstractColumn $col)
    {
        if ($col instanceof SimpleColumn) {
            if (!in_array($col, $groupColumns)) {
                $messageToLogAndUser = MessagesEnum::getMessageWithArgs(
                    MessagesEnum::ADD_COL_TO_GROUP_BY_OR_AGG,
                    $this->localeForUserMessages,
                    $col->getId()
                );
                //$log->error($messageToLogAndUser);
        ...
        } elseif ($col instanceof ScalarFunctionColumn) {
            if (!in_array($col, $groupColumns)) {
        ...
        if (strpos($s, "\\") !== false) {
            if (strpos($s, "'") !== false) {
                throw new \RuntimeException(
                    'Cannot represent string that contains both double-quotes (") and single quotes (\').'
                );
            } else {
                return "'" . $s . "'";
            }
