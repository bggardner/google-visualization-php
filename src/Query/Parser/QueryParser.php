...
class QueryParser
{
    protected const DATE_FORMAT = '[0-9]{4}-[0-9]{2}-[0-9]{2}';
    protected const NOT_BACK_QUOTED = '(?=(?:[^`]*`[^`]*`)*[^`]*$)';
    protected const NAMED_SCALAR_FUNCTIONS_REGEXP =
        '(year)|(month)|(day)|(hour)|(minute)|(second)|(millisecond)|(quarter)|(dayOfWeek)|(now)|(dateDiff)|(toDate)'
        . '|(upper)|(lower)|(concat)|(concat_ws)|(abs)|(round)|(right)|(left)';
    protected const AGGREGATION_FUNCTIONS_REGEXP = '(count)|(sum)|(min)|(max)|(avg)';
    protected const TIME_FORMAT = '[0-9]{2}:[0-9]{2}:[0-9]{2}(?:.[0-9]{0-3})?';
    protected const COMPARISON_REGEXP =
        '(<=)|(?:(<)[^=>])|(?:[^<](>)[^=])|(>=)|(?:[^!<>](=))|(!=)|(<>)'
        . '|(?:\s+(?:(contains)|(starts with)|(ends with)|(matches)|(like))\s+)';
    protected const UNQUOTED_LOOKAHEAD = '(?=(?:[^"]*"[^"]*")*[^"]*$)(?=(?:[^\']*\'[^\']*\')*[^\']*$)';
    protected const VALUE_PATTERN =
        '/^(?:\(?("(?:[^"]|(?:""))*")|(\'(?:[^\']|(?:\'\'))*\')|(-?[0-9]*\.?[0-9]+)|(true|false)'
        . '|((?:date|timeofday|datetime)\s+(?:(?:"[^"]*")|(?:\'[^\']*\')))\)?)$/i';
    ...
    protected static function parseFilter($argumentString)
    {
        ...
        for ($i = 0; $i < strlen($argumentString); $i++) {
            ...
            if (
                $c == '('
                 && preg_match(
                     '/' . self::AGGREGATION_FUNCTIONS_REGEXP . '|' . self::NAMED_SCALAR_FUNCTIONS_REGEXP . '$/i',
                     $innerExp
                 )
            ) {
                $sfCount++;
        ...
        while (
            preg_match(
                '/(?<!is)((^|\s+)not\s*)(\{\d+\})?' . self::UNQUOTED_LOOKAHEAD . '/i',
                $outerExp,
                $matches,
                PREG_OFFSET_CAPTURE
            )
            ...
            if (
                preg_match(
                    '/\s+(and|or)\s+' . self::UNQUOTED_LOOKAHEAD . '/i',
                    $notExp,
                    $matches,
                    PREG_OFFSET_CAPTURE
                )
            ...
            if (preg_match('/\{(\d+)\}' . self::UNQUOTED_LOOKAHEAD . '/', $notExp, $matches)) {
                ...
                $outerExp = substr($outerExp, 0, $notOffset);
                $outerExp .= ' {' . $key . '} ';
                $outerExp .= substr($outerExp, $notOffset + strlen($notMatch));
            } else {
                $a[] = new NegationFilter(self::parseNonCompoundFilter($notExp));
                $outerExp = substr($outerExp, 0, $notOffset);
                $outerExp .= ' {' . (count($a) ? max(array_keys($a)) : 0) . '} ';
                $outerExp .= substr($outerExp, $notOffset + strlen($notMatch) + $loOffset);
            }
        }
        ...
        while (preg_match('/\s+and\s+' . self::UNQUOTED_LOOKAHEAD . '/i', $outerExp, $matches, PREG_OFFSET_CAPTURE)) {
            ...
            if (
                preg_match(
                    '/\s+or\s+' . self::UNQUOTED_LOOKAHEAD . '/i',
                    substr($outerExp, 0, $andOffset),
                    $matches,
                    PREG_OFFSET_CAPTURE
                )
            ) {
            ...
            $outerExp = substr($outerExp, 0, $startOffset);
            $outerExp .= ' {' . (count($a) ? max(array_keys($a)) : 0) . '} ';
            $outerExp .= substr($outerExp, $endOffset);
        }
    ...
    protected static function parseNonCompoundFilter($filterString)
    {
        if (preg_match('/' . self::COMPARISON_REGEXP . self::UNQUOTED_LOOKAHEAD . '/i', $filterString, $matches)) {
            ...
            if (preg_match(self::VALUE_PATTERN, trim($args[0]), $matches)) {
                $matches = array_values(array_filter($matches, 'strlen'));
                $filter = new ColumnValueFilter(
                    self::parseColumn($args[0]),
                    self::parseValue($matches[1]),
                    $operator,
                    true
                );
            } elseif (preg_match(self::VALUE_PATTERN, trim($args[1]), $matches)) {
                $matches = array_values(array_filter($matches, 'strlen'));
                $filter = new ColumnValueFilter(
                    self::parseColumn($args[0]),
                    self::parseValue($matches[1]),
        ...
        } elseif (preg_match('/(.*)\sis\s(?:not\s)?null' . self::UNQUOTED_LOOKAHEAD . '/i', $filterString, $matches)) {
    ...
    protected static function parseValue($valueString)
    {
        ...
        if (preg_match('/^true|false$/i', $valueString)) {
        ...
        } elseif (preg_match('/^-?[0-9]*\.?[0-9]+$/', $valueString)) {
            $value = new NumberValue($valueString);
        } elseif (preg_match('/^(?:(?:"((?:[^"]|(?:""))*)")|(?:\'((?:[^\']|(\'\'))*)\'))$/', $valueString, $matches)) {
            $matches = array_values(array_filter($matches, "strlen"));
            $value = $matches[1];
            // Unescape double-quotes if double-quoted string
            $value = strpos($valueString, '"') === 0 ? str_replace('""', '"', $value) : $value;
            // Unescape single-quotes if single-quoted string
            $value = strpos($valueString, "'") === 0 ? str_replace("''", "'", $value) : $value;
            $value = new TextValue($value);
        } elseif (
            preg_match(
                '/^date\s+(?:(?:"(' . self::DATE_FORMAT . ')")|(?:\'(' . self::DATE_FORMAT . ')\'))$/',
                $valueString,
                $matches
            )
        ) {
            ...
            preg_match(
                '/^timeofday\s+(?:(?:"(' . self::TIME_FORMAT . ')")|(?:\'(' . self::TIME_FORMAT . ')\'))$/',
                $valueString,
                $matches
            )
        ) {
            ...
            preg_match(
                '/^datetime\s+(?:(?:"(' . self::DATE_FORMAT . '\s+' . self::TIME_FORMAT . ')")'
                . '|(?:\'(' . self::DATE_FORMAT . '\s+' . self::TIME_FORMAT . ')\'))$/',
                $valueString,
                $matches
            )
        ) {
    ...
    protected static function splitArguments($str, $pattern = '/,/')
    {
        ...
        for ($i = 0; $i < strlen($str); $i++) {
            ...
            if ($c == '`') {
                ...
                $sqEscaped = !$sqEscaped;
            } elseif (!$bqEscaped && !$dqEscaped && !$sqEscaped) {
                if ($c == ')') {
                    $parensCount--;
                } elseif ($c == '(') {
                    $parensCount++;
                } elseif ($parensCount == 0 && preg_match($pattern, $c)) {
                    $word = trim($word);
                    if (strlen($word) > 0) {
                        $a[] = $word;
                    }
    ...
    protected static function parseColumn($arg)
    {
        ...
        if (preg_match('/^`([^`]+)`$/', $arg, $matches)) { // Back-quoted identifier
            ...
            preg_match(
                '/^(([^\(]+)|([^\(]*\([^\)]*\)))\s*[\+\-\*\/%]' . self::UNQUOTED_LOOKAHEAD . '/',
                $arg
            )
        ) { // Arithmetic expression not enclosed in a function
            $operators = self::getOuterOperators($arg);
            // Operator precedence
            if (preg_match('/[\*\/%]/', implode('', $operators))) { // Multiply, divide, or modulo
                $column = self::parseArithmeticColumn($arg, '/[\*\/%]/');
            } elseif (preg_match('/[\+\-]/', implode('', $operators))) { // Add or subtract
                $column = self::parseArithmeticColumn($arg, '/[\+\-]/');
            }
        } elseif (preg_match(self::VALUE_PATTERN, $arg, $matches)) { // Constant
            $matches = array_values(array_filter($matches, "strlen"));
            $column = new ScalarFunctionColumn(array(), new Constant(self::parseValue($matches[1])));
        } elseif (($parenPos = strpos($arg, '(')) === false) { // Not a function
            if ($arg == '') {
                throw new InvalidQueryException('Column name is required.');
            }
            ...
            preg_match(
                '/^' . self::AGGREGATION_FUNCTIONS_REGEXP
                . '|' . self::NAMED_SCALAR_FUNCTIONS_REGEXP
                . '\\([^)]*\\)' . self::UNQUOTED_LOOKAHEAD . '/i',
                $arg
            )
        ) { // Named function
            $parenPos = strpos($arg, '(');
            ...
            if (defined($aggTypeString)) { // Aggregation Function
                ...
                if (!($aggregatedColumn instanceof SimpleColumn)) {
                    throw new InvalidQueryException(
                        $colFunc . '() does not support expressions.  Only column names can be used.'
                    );
                }
                ...
                switch ($colFunc) {
                    ...
                    case 'SECOND':
                    case 'MILLISECOND':
                        $scalarFunction = new TimeComponentExtractor(
                            constant('Google\Visualization\DataSource\Query\ScalarFunction\TimeComponent::' . $colFunc)
                        );
                        break;
                    case 'NOW':
