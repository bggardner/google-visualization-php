...
{
    protected const SAME_ORIGIN_HEADER = 'X-DataSource-Auth';
    protected const QUERY_REQUEST_PARAMETER = 'tq';
    protected const DATASOURCE_REQUEST_PARAMETER = 'tqx';
    ...
    public static function getDefaultDataSourceRequest()
    {
        ...
        try {
            ...
            if (
                ($dataSourceRequest->dsParams->getOutputType() == OutputType::JSON)
    ...
    public static function determineSameOrigin()
    {
        ...
        $sameOrigin |= array_key_exists(
            'HTTP_' . strtoupper(str_replace('-', '_', self::SAME_ORIGIN_HEADER)),
            $_SERVER
        );
        return $sameOrigin;
    }
    ...
    {
        $this->dsParams = new DataSourceParameters(
            isset($_REQUEST[self::DATASOURCE_REQUEST_PARAMETER]) ? $_REQUEST[self::DATASOURCE_REQUEST_PARAMETER] : ''
        );
